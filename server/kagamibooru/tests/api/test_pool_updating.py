...
from kagamibooru.func import pools, posts, snapshots
...
def test_simple_updating(user_factory, pool_factory, context_factory):
    ...
    db.session.add(pool)
    db.session.commit()
    with patch("kagamibooru.func.pools.create_pool"), patch(
        "kagamibooru.func.posts.get_posts_by_ids"
    ), patch("kagamibooru.func.pools.update_pool_names"), patch(
        "kagamibooru.func.pools.update_pool_category_name"
    ), patch(
        "kagamibooru.func.pools.update_pool_description"
    ), patch(
        "kagamibooru.func.pools.update_pool_posts"
    ), patch(
        "kagamibooru.func.pools.serialize_pool"
    ), patch(
...
def test_omitting_optional_field(
    ...
    del params[field]
    with patch("kagamibooru.func.pools.create_pool"), patch(
        "kagamibooru.func.pools.update_pool_names"
    ), patch("kagamibooru.func.pools.update_pool_category_name"), patch(
        "kagamibooru.func.pools.serialize_pool"
    ):
...
def test_trying_to_create_pools_without_privileges(
    ...
    with patch("kagamibooru.func.posts.get_posts_by_ids"):
