...
from kagamibooru.func import pools, snapshots
...
def test_merging(user_factory, pool_factory, context_factory, post_factory):
    ...
    assert target_pool.post_count == 0
    with patch("kagamibooru.func.pools.serialize_pool"), patch(
        "kagamibooru.func.pools.merge_pools"
    ), patch("kagamibooru.func.snapshots.merge"):
