...
from kagamibooru.func import snapshots, tags
...
def test_simple_updating(user_factory, tag_factory, context_factory):
    ...
    db.session.add(tag)
    db.session.commit()
    with patch("kagamibooru.func.tags.create_tag"), patch(
        "kagamibooru.func.tags.get_or_create_tags_by_names"
    ), patch("kagamibooru.func.tags.update_tag_names"), patch(
        "kagamibooru.func.tags.update_tag_category_name"
    ), patch(
        "kagamibooru.func.tags.update_tag_description"
    ), patch(
        "kagamibooru.func.tags.update_tag_suggestions"
    ), patch(
        "kagamibooru.func.tags.update_tag_implications"
    ), patch(
        "kagamibooru.func.tags.serialize_tag"
    ), patch(
        "kagamibooru.func.snapshots.modify"
    ):
...
def test_omitting_optional_field(
    ...
    del params[field]
    with patch("kagamibooru.func.tags.create_tag"), patch(
        "kagamibooru.func.tags.update_tag_names"
    ), patch("kagamibooru.func.tags.update_tag_category_name"), patch(
        "kagamibooru.func.tags.serialize_tag"
    ):
...
def test_trying_to_create_tags_without_privileges(
    ...
    with patch("kagamibooru.func.tags.get_or_create_tags_by_names"):
