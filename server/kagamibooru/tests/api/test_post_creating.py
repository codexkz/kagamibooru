...
from kagamibooru import api, db, errors, model
from kagamibooru.func import net, posts, snapshots, tags
...
def test_creating_minimal_posts(context_factory, post_factory, user_factory):
    ...
    with patch("kagamibooru.func.posts.create_post"), patch(
        "kagamibooru.func.posts.update_post_safety"
    ), patch("kagamibooru.func.posts.update_post_source"), patch(
        "kagamibooru.func.posts.update_post_relations"
    ), patch(
        "kagamibooru.func.posts.update_post_notes"
    ), patch(
        "kagamibooru.func.posts.update_post_flags"
    ), patch(
        "kagamibooru.func.posts.update_post_thumbnail"
    ), patch(
        "kagamibooru.func.posts.serialize_post"
    ), patch(
        "kagamibooru.func.snapshots.create"
    ):
...
def test_anonymous_uploads(
    ...
    with patch("kagamibooru.func.posts.serialize_post"), patch(
        "kagamibooru.func.posts.create_post"
    ), patch("kagamibooru.func.posts.update_post_source"), patch(
        "kagamibooru.func.snapshots._post_to_webhooks"
    ):
        config_injector(
            {
                "privileges": {
...
def test_creating_from_url_saves_source(
    ...
    with patch("kagamibooru.func.net.download"), patch(
        "kagamibooru.func.posts.serialize_post"
    ), patch("kagamibooru.func.posts.create_post"), patch(
        "kagamibooru.func.posts.update_post_source"
    ), patch(
        "kagamibooru.func.snapshots._post_to_webhooks"
    ):
        config_injector(
...
def test_errors_not_spending_ids(
    ...
    # successful request
    with patch("kagamibooru.func.posts.serialize_post"), patch(
        "kagamibooru.func.posts.update_post_tags"
    ), patch("kagamibooru.func.snapshots._post_to_webhooks"):
...
def test_trying_to_create_tags_without_privileges(
    ...
    with pytest.raises(errors.AuthError), patch(
        "kagamibooru.func.posts.update_post_content"
    ), patch("kagamibooru.func.posts.update_post_tags"):
