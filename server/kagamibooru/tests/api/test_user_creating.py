...
from kagamibooru import api, errors, model
...
def test_creating_user(user_factory, context_factory, fake_datetime):
    user = user_factory()
    with patch("kagamibooru.func.users.create_user"), patch(
        "kagamibooru.func.users.update_user_name"
    ), patch("kagamibooru.func.users.update_user_password"), patch(
        "kagamibooru.func.users.update_user_email"
    ), patch(
        "kagamibooru.func.users.update_user_rank"
    ), patch(
        "kagamibooru.func.users.update_user_avatar"
    ), patch(
        "kagamibooru.func.users.serialize_user"
    ), fake_datetime(
        "1969-02-12"
    ):
...
def test_trying_to_omit_mandatory_field(user_factory, context_factory, field):
    ...
    del params[field]
    with patch("kagamibooru.func.users.create_user"), pytest.raises(
...
def test_omitting_optional_field(user_factory, context_factory, field):
    ...
    with patch("kagamibooru.func.users.create_user"), patch(
        "kagamibooru.func.users.update_user_avatar"
    ), patch("kagamibooru.func.users.serialize_user"):
        users.create_user.return_value = user
        api.user_api.create_user(
            context_factory(params=params, user=auth_user)
        )
