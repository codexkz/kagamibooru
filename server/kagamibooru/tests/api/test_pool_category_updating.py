...
from kagamibooru.func import pool_categories, snapshots
...
def test_simple_updating(user_factory, pool_category_factory, context_factory):
    ...
    db.session.flush()
    with patch("kagamibooru.func.pool_categories.serialize_category"), patch(
        "kagamibooru.func.pool_categories.update_category_name"
    ), patch("kagamibooru.func.pool_categories.update_category_color"), patch(
...
def test_set_as_default(user_factory, pool_category_factory, context_factory):
    ...
    db.session.commit()
    with patch("kagamibooru.func.pool_categories.serialize_category"), patch(
        "kagamibooru.func.pool_categories.set_default_category"
    ):
        pool_categories.update_category_name.side_effect = (
            _update_category_name
        )
