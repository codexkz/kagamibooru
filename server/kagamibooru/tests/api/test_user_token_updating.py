...
from kagamibooru import api, db
from kagamibooru.func import user_tokens, users
...
def test_edit_user_token(user_token_factory, context_factory, fake_datetime):
    ...
    db.session.commit()
    with patch("kagamibooru.func.user_tokens.get_by_user_and_token"), patch(
        "kagamibooru.func.user_tokens.update_user_token_enabled"
    ), patch("kagamibooru.func.user_tokens.update_user_token_edit_time"), patch(
        "kagamibooru.func.user_tokens.serialize_user_token"
    ), patch(
        "kagamibooru.func.users.get_user_by_name"
    ), fake_datetime(
        "1969-02-12"
    ):
