...
from kagamibooru.func import comments
...
def test_simple_rating(
    ...
    db.session.commit()
    with patch("kagamibooru.func.comments.serialize_comment"), fake_datetime(
        "1997-12-01"
    ):
...
def test_updating_rating(
    ...
    db.session.add(comment)
    db.session.commit()
    with patch("kagamibooru.func.comments.serialize_comment"):
