...
from kagamibooru.func import snapshots, tag_categories, tags
...
def test_simple_updating(user_factory, tag_category_factory, context_factory):
    ...
    db.session.flush()
    with patch("kagamibooru.func.tag_categories.serialize_category"), patch(
        "kagamibooru.func.tag_categories.update_category_name"
    ), patch("kagamibooru.func.tag_categories.update_category_color"), patch(
...
def test_set_as_default(user_factory, tag_category_factory, context_factory):
    ...
    db.session.commit()
    with patch("kagamibooru.func.tag_categories.serialize_category"), patch(
        "kagamibooru.func.tag_categories.set_default_category"
    ):
        tag_categories.update_category_name.side_effect = _update_category_name
        tag_categories.serialize_category.return_value = "serialized category"
