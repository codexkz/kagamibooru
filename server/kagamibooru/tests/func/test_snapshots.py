...
from kagamibooru.func import snapshots, users
...
def test_serialize_snapshot(user_factory):
    ...
    with patch("kagamibooru.func.users.serialize_micro_user"):
...
def test_create(tag_factory, user_factory):
    ...
    db.session.add(tag)
    db.session.flush()
    with patch("kagamibooru.func.snapshots.get_tag_snapshot"), patch(
...
def test_merge(tag_factory, user_factory):
    ...
    db.session.flush()
    with patch("kagamibooru.func.snapshots._post_to_webhooks"):
