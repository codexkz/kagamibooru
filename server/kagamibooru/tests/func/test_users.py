...
from kagamibooru.func import auth, files, users, util
...
def test_get_email(
    ...
    with patch("kagamibooru.func.auth.has_privilege"):
...
def test_serialize_user(user_factory):
    with patch("kagamibooru.func.users.get_email"), patch(
        "kagamibooru.func.users.get_avatar_url"
    ), patch("kagamibooru.func.users.get_liked_post_count"), patch(
        "kagamibooru.func.users.get_disliked_post_count"
    ):
...
def test_serialize_micro_user(user_factory):
    with patch("kagamibooru.func.users.get_avatar_url"):
        users.get_avatar_url.return_value = "https://example.com/avatar.png"
        auth_user = user_factory()
        user = user_factory(name="dummy user")
        db.session.add(user)
...
def test_create_user_for_first_user(fake_datetime):
    with patch("kagamibooru.func.users.update_user_name"), patch(
        "kagamibooru.func.users.update_user_password"
    ), patch("kagamibooru.func.users.update_user_email"), fake_datetime(
        "1997-01-01"
    ):
...
def test_create_user_for_subsequent_users(user_factory, config_injector):
    ...
    db.session.flush()
    with patch("kagamibooru.func.users.update_user_name"), patch(
        "kagamibooru.func.users.update_user_email"
    ), patch("kagamibooru.func.users.update_user_password"):
        user = users.create_user("name", "password", "email")
...
def test_update_user_name_reusing_own_name(user_factory, config_injector):
    ...
    db.session.add(user)
    db.session.flush()
    with patch("kagamibooru.func.files.has"):
...
def test_update_user_name_moves_avatar(user_factory, config_injector):
    ...
    with patch("kagamibooru.func.files.has"), patch(
        "kagamibooru.func.files.move"
    ):
...
def test_update_user_password(user_factory, config_injector):
    config_injector({"password_regex": "^[a-z]+$"})
    user = user_factory()
    with patch("kagamibooru.func.auth.create_password"), patch(
        "kagamibooru.func.auth.get_password_hash"
    ):
...
def test_update_user_email_with_invalid_email(user_factory):
    user = user_factory()
    with patch("kagamibooru.func.util.is_valid_email"):
...
def test_update_user_avatar_to_empty_manual(user_factory):
    user = user_factory()
    with patch("kagamibooru.func.files.has"), pytest.raises(
...
def test_update_user_avatar_to_new_manual(user_factory, config_injector):
    ...
    user = user_factory()
    with patch("kagamibooru.func.files.save"):
