...
from kagamibooru import db, model
...
def test_serialize_post(
    ...
    config_injector({"data_url": "http://example.com/", "secret": "test"})
    with patch("kagamibooru.func.comments.serialize_comment"), patch(
        "kagamibooru.func.users.serialize_micro_user"
    ), patch("kagamibooru.func.posts.files.has"):
...
def test_serialize_micro_post(post_factory, user_factory):
    with patch("kagamibooru.func.posts.get_post_thumbnail_url"):
...
def test_create_post(user_factory, fake_datetime):
    with patch("kagamibooru.func.posts.update_post_content"), patch(
...
def test_update_post_content_for_new_post(
    ...
    with patch("kagamibooru.func.util.get_sha1"), patch(
        "kagamibooru.func.util.get_md5"
    ):
