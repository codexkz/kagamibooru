...
from kagamibooru.func import cache, tag_categories, tags
...
def test_create_tag(fake_datetime):
    with patch("kagamibooru.func.tags.update_tag_names"), patch(
        "kagamibooru.func.tags.update_tag_category_name"
    ), patch("kagamibooru.func.tags.update_tag_suggestions"), patch(
...
def test_update_tag_category_name(tag_factory):
    with patch("kagamibooru.func.tag_categories.get_category_by_name"):
...
def test_update_tag_suggestions(tag_factory):
    tag = tag_factory(names=["name", "ALIAS"])
    with patch("kagamibooru.func.tags.get_tags_by_names"):
